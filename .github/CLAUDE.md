...
2. The constructor takes `page: Page` and stores it in a field.
3. **Every locator is initialized in the constructor** and stored in `readonly Locator` fields. Do not build locators inside methods.
...
- `readonly` on every locator field of a Page Object.
...
2. Locators — CSS by `data-marker` / XPath, stored as `readonly` fields, initialized in the constructor.
3. Test data with multiple cases → declare an `interface` in [types/](types/) and use `for...of` with destructuring. Never inline the type in the spec.
4. Reusable utility / external-library wrapper → put it in [Helpers/](Helpers/), not next to the test.
5. Every step of the scenario is wrapped in `await test.step('...', async () => { ... })`.
6. `test` / `expect` are imported from [test-options.ts](test-options.ts).
7. Test is tagged `{ tag: '@<feature>' }` and sits inside `test.describe('<Feature> functionality', ...)`.
8. No direct `page.goto` or raw locators in the spec — everything goes through `app.<pageObject>`.
